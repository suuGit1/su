...
class PowerIoTVPPEnvironment:
    ...
    def __init__(self, config: VPPEnvConfig | None = None) -> None:
        ...
        self.twin = DigitalTwinSynchronizer(TwinConfig(dt_hours=self.config.dt_hours))
    ...
    def observe(self, use_digital_twin: bool = True) -> dict[str, float]:
        ...
        if use_digital_twin:
            ...
            obs = self.twin.aggregate_state()
        else:
            obs = self._aggregate_raw_observations(observations)
    ...
    def step(self, action: dict[str, float], apply_safety: bool = True, use_grid: bool | None = None) -> StepResult:
        ...
        curtailment_kw = max(0.0, self.true_state["pv_kw"] + self.true_state["wind_kw"] - self.true_state["load_kw"] - abs(applied.get("ess_power_kw", 0.0)))
        grid_power_kw = applied.get(
            "grid_power_kw",
            self.true_state["load_kw"] - self.true_state["pv_kw"] - self.true_state["wind_kw"] - applied.get("ess_power_kw", 0.0) - applied.get("ev_power_kw", 0.0) - applied.get("dr_kw", 0.0),
        )

        communication_cost = self.channel.total_communication_cost()
        ...
        violations = 0 if safety_report is None else safety_report.violation_count
    ...
    def _next_soc(self, soc: float, power_kw: float, capacity_kwh: float) -> float:
        if power_kw >= 0:
            soc -= power_kw * self.config.dt_hours / (capacity_kwh * 0.95)
        else:
            soc += abs(power_kw) * self.config.dt_hours * 0.95 / capacity_kwh
