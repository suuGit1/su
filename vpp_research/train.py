...
from .environment import ResearchEnv,OBS_VERSION,OBJECTIVE_VERSION,SCALES
from .pareto_agent import ParetoAgent,sample_preference,VERSION


def train(config,dt_model,output,method='pareto',preference=(1.,0.,0.),robust=False,scenario_offset=0):
    config.validate()
    if config.algorithm!='mappo' or config.device!='cpu':raise ValueError('本研究驱动当前验证了 CPU MAPPO；旧 GPU/IPPO 入口继续独立保留')
    if method not in ('ordinary','fixed','pareto'):raise ValueError('未知学习方法')
    ...
    env=ResearchEnv(config,config.train_csv,dt_model,robust);rng=np.random.default_rng(config.seed+31415)
    agent=ParetoAgent(65,env.num_agents,config.hidden_size,config.lr) if method=='pareto' else OfficialPPO(config,env)
    history=[];weights=[]
    for ep in range(config.episodes):
        ...
        if method=='pareto':roll=dict(obs=[],preferences=[],actions=[],logprobs=[],values=[],vectors=[],dones=[])
        else:
            buffer=agent.buffer();buffer.obs[0,0]=obs;buffer.share_obs[0,0]=share
        for t in range(config.horizon):
            if method=='pareto':a,lp,v=agent.act(obs,w)
            else:
                ...
                a=to_numpy(a)
            no,ns,reward,done,info=env.step(a);vector=np.array(info['objective_vector']);vectors.append(vector);violations+=info['constraint_violations']
            if method=='pareto':
            ...
            obs,share=no,ns
        if method=='pareto':metrics=agent.update(roll,config.ppo_epoch,config.clip_param,config.gamma,config.gae_lambda,config.entropy_coef)
        else:
    ...
    state=dict(version=VERSION,observation_version=OBS_VERSION,objective_version=OBJECTIVE_VERSION,scales=SCALES.tolist(),
    ...
    state['agent']=agent.state_dict() if method=='pareto' else agent.state()
...
def load(checkpoint):
    ...
    if (state['version'],state['observation_version'],state['objective_version'])!=(VERSION,OBS_VERSION,OBJECTIVE_VERSION):raise ValueError('研究检查点协议不匹配')
    ...
    c.train_csv=None;env=ResearchEnv(c,dt_model=state['dt_model'],robust=state['robust'])
    agent=ParetoAgent(65,env.num_agents,c.hidden_size,c.lr) if state['method']=='pareto' else OfficialPPO(c,env)
    if state['method']=='pareto':agent.load_state_dict(state['agent']);agent.eval()
...
def evaluate(checkpoint,preferences,seeds,output=None,robust=None,stress=None,csv_path=None,ev_bundle=None,ac_safe=False):
    ...
    env=ResearchEnv(c,csv_path,state['dt_model'],state['robust'] if robust is None else robust,ac_safe=ac_safe)
        ...
        for index,seed in enumerate(seeds):
            ...
            try:
                ...
                for _ in range(c.horizon):
                    if state['method']=='pareto':a,_,_=agent.act(obs,w,True)
                    else:
                ...
                row.update(vector=np.sum([i['objective_vector'] for i in infos],axis=0).tolist(),
