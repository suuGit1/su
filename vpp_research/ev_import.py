...
def convert(raw,output,dt_hours=.25,max_kw=7.,source='https://ev.caltech.edu/dataset'):
    ...
    scenarios={};excluded=Counter();kept=[];h=round(24/dt_hours);seconds=dt_hours*3600
    for item in items:
        try:
    ...
    result=dict(schema_version=1,energy_basis='grid_kwh',source=source,horizon=h,dt_hours=dt_hours,scenarios=scenarios)
    p=Path(output)
    ...
    audit=dict(source=source,raw_sha256=hashlib.sha256(Path(raw).read_bytes()).hexdigest(),input_count=len(items),accepted=len(kept),excluded=dict(excluded),
        assumptions=dict(max_kw=max_kw,departure='申报离站用于规划；真实提前离站尚未进入动态回放，不能声称完成真实出行验证',availability='到站向上取整，申报离站向下取整'),records=kept)
    p.with_suffix('.audit.json').write_text(json.dumps(audit,ensure_ascii=False,indent=2));return audit

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--input',required=True);p.add_argument('--output',required=True);p.add_argument('--max-kw',type=float,required=True);p.add_argument('--dt-hours',type=float,default=.25);a=p.parse_args();print(convert(a.input,a.output,a.dt_hours,a.max_kw))
